...
class SetNumStatusWidgets extends Widget {
    //定义属性
    public $view;
    public $del=true;
    ...
    public function run(){
        ...
        $js=<<<MYJS
//批量设置状态为某个值（不是状态ID），kn=状态值是第几个列字段？（成功会将值修改为设置值）
function setAll(statusNum,statusValue,kn){
    //注意这里的$("#grid")，要跟我们GridView::widget设定的 id一致
    var keys = $("#grid").yiiGridView("getSelectedRows");
    if(keys!=""){
        $.post("$status_url",{ids:keys,status:statusNum},function (data, status) {
            if (status == "success"){
                if ((data instanceof Object) && !(data instanceof Array)){
                   if(('status' in data) && ('msg' in data)){
                       if(data.status==0){
                           $("#grid tr").each(function (key, domEle) {
