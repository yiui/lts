...
class UploadImg {
    ...
    public function upConfig($config){
        ...
        $default_config=array(
            //不同类型保存路径不同
            'item'=>Pic::ITEM_DEFAULT,//可以使用名称也可以使用类型ID号
            ...
            'quality'=>80,//保存图片后的质量：save('11_rotate.jpg', ['quality' => $this->config['quality']]);
...
//        $this->config['url'] = rtrim(ltrim($this->config['path'], '@'),'/') . '/';//注意，处理添加进配置中的url前缀
     ...
     * @param string $att 表单名或叫接受文件的模型属性
     * @param string $toatt    接受文件ID的模型属性，多个文件可以使用数组哦:$toname='pic_id[]'
        ...
        if ($model->$att) {
            ...
            foreach ($allfile as $k => $file) {
                ...
                if ($file->saveAs($this->config['path']. $this->upfile[$att][$k]['name'])){
                    $this->upfile[$att][$k]['path']=$this->config['path'] . $this->upfile[$att][$k]['name'];//绝对路径
                    $this->upfile[$att][$k]['url']=$this->config['url'] . $this->upfile[$att][$k]['name'];//文件名含后缀
